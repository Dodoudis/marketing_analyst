    ...
    while True:
        ...
        if response.stop_reason == "tool_use":
            if tool_rounds >= MAX_TOOL_ROUNDS:
                return (
                    "I reached the maximum number of tool calls. Try rephrasing your question.",
                    tool_log,
                    last_sql,
                )
...
with st.sidebar:
    ...
    <div class="agent-header">
        <div>
            ...
            <p class="agent-subtitle">POWERED BY CLAUDE + BIGQUERY</p>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
for msg in st.session_state.display_messages:
    if msg["role"] == "user":
        ...
        # Show dataframe if result contains tabular data
...
# ---------------------------------------------------------------------------
# Handle input — from chat box or sidebar button
# ---------------------------------------------------------------------------
...
if user_input:
    ...
    # Try to extract a dataframe from the last successful query
    df_result = None
